...
Первый практический срез — **не «только backend»**. Локальный Docker: этапы 0+1+2 **и минимальный фронт** (токен, логин, форма поста, голос с Bearer, доска `study`). Иначе `requireAuth` на vote и serialize без `comments` ломают текущий `App.vue`.

Детальный чеклист локали (compose, env, `node_modules` в контейнере, сиды): [first-slice-local.md](first-slice-local.md).

Следующая сессия после этого среза — этапы 3+4 (Vue Router, список досок, тред).
